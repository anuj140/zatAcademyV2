...
const { validateSessionTiming, sendSessionNotifications } = require('../utils/sessionUtils');
...
exports.createLiveSession = async (req, res) => {
  try {
    ...
    if (batch.instructor.toString() !== req.user.id && req.user.role !== 'admin' && req.user.role !== 'superAdmin') {
...
exports.updateLiveSession = async (req, res) => {
  try {
    ...
    if (req.body.startTime || req.body.endTime) {
      const newStartTime = req.body.startTime ? new Date(req.body.startTime) : session.startTime;
